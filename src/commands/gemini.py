...
ERROR_MESSAGES = {Errors.FILE_UPLOAD_ERR: 'There was an error uploading the file.',
...
class GeminiBot:
    ...
    def prompt_gemini(self, input_msg=None, attachment=None) -> (Embed, Errors):
        ...
        response_embed.add_field(name="Input", value=input_msg if input_msg is not None else "Attachment", inline=False)
    ...
    async def query(self, author, message=None, attachment=None) -> List[Embed]:
        ...
        if not message and not attachment:
            response_embed = self.prompt_gemini(f"Roast the user using their username - {author} for providing a blank input.")
            return [response_embed]
        
        # Message too long
        if message is not None and len(message) > 200:
            response_embed = self.prompt_gemini(f"Roast the user using their username - {author} for providing way too many tokens.")
            return [response_embed]
        ...
        if message is None and err is not None:
            return [Embed(title="Ask Gemini", description = "The attachment is invalid and no text input is provided.", color = 0xFF3333)]
        
        response_embed, err = self.prompt_gemini(input_msg=message, attachment=attachment_ref)
        ...
        if err is not None:
            err_embed = Embed(title="NOTE", description=ERROR_MESSAGES[err], color = 0xFF3333)
...
async def upload_or_return_file_ref(attachment) -> (File, Errors):
    ...
    try:
        ...
        # Upload image and get ref
            ...
            file_ref = genai.upload_file(path=f"temp/{attachment.filename}", name=file_name)
...
def is_valid_ext_size(file) -> Errors:
    ...
    valid_content_types = ['image/png', 'image/jpeg', 'image/webp', 'image/heic', 'image/heif',
        'audio/wav', 'audio/mp3', 'audio/aiff', 'audio/aac', 'audio/ogg', 'audio/flac']
    print("Verifying attachment ", file.content_type)

    # File too large
    if (file.is_voice_message() and file.duration() > 300) or file.size > 3e+7:
